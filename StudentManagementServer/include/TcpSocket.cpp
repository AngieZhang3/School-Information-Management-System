...
int TcpSocket::Send(TcpPcb *pPcb, char *pBuf, int nLen)
{
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      */
      ...
      LOGD("[DEBUG] cmd:%d seq:%d segment:%d nLen:%d nSent:%d SendQueue:%d SendBuf:%d\r\n", 
          p->nCmd, 
          p->nSeq, 
          p->nSegment, 
          p->nLen, 
...
int TcpSocket::Input(TcpPcb *pPcb)
{
  ...
  switch (((TcpPacket*)RcvBuf)->nCmd) {
    case CMD_PSH: {
      ...
      LOGD("[DEBUG] cmd:PSH seq:%d segment:%d nLen:%d RcvBuf:%d\r\n", 
          p->nSeq, 
          p->nSegment, 
          p->nLen, 
          pPcb->RecvBuf.size());
...
int TcpSocket::Output(TcpPcb *pPcb, TcpPacket* p)
{
   ...
   if (nSent < 0) {
     ...
     LOGD("[DEBUG] %s cmd:%d seq:%d segment:%d nLen:%d nSent:%d\r\n",
          __FUNCTION__,
          p->nCmd, 
          p->nSeq, 
          p->nSegment, 
